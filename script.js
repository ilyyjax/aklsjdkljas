...
document.addEventListener('keyup', e => {
  if (e.code === 'Space') {
    ...
    objects = [];
    // Remove milestone lines
    document.querySelectorAll('.milestone-line').forEach(line => line.remove());
  }
});
...
function createMilestoneLine(value) {
  const line = document.createElement('div');
  line.className = 'milestone-line';
  const gameHeight = gameArea.clientHeight;
  line.style.bottom = Math.min(gameHeight - 3, value / 1000 * 6) + 'px'; // approximate position
...
function gameLoop() {
  if (holding) {
    ...
    if (Math.random() < 0.0075) spawnCloud(gameHeight + Math.random() * 200, Math.random() * 0.5 + 0.5); // 25% of original clouds
...
// Create initial milestone lines
milestones.forEach(value => createMilestoneLine(value));
for (let i = 100000; i <= 1000000; i += 100000) createMilestoneLine(i);
